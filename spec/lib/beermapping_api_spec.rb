...
it "When HTTP GET returns no entries, empty list is returned." do
  ...
  stub_request(:get, /.*unknown/).to_return(body: canned_answer, headers: { 'Content-Type' => "text/xml" })

  BeermappingApi.places_in("unknown")

  places = BeermappingApi.places_in("unknown")
  ...
  it "When HTTP GET returns multiple entries, they are parsed and returned" do
      ...
      helsinki = BeermappingApi.places_in("helsinki")

      expect(helsinki.size).to eq 7

      first = helsinki.first
